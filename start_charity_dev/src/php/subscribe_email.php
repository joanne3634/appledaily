...
while( $result = $query->fetch(PDO::FETCH_ASSOC) ){
	...
	if( $donater_email != 'na' && ($days == null || $days > $freq) ){
		...
		sendEmail( $mail, $donater_email, $email_body, $dba, $fb_id );
	}
...
function sendEmail( $mail, $donater_email, $email_body, $dba, $fb_id ){
	...
	$mail->addAddress( 'joanne3634@gmail.com' , encoding($donater_email) );     // Add a recipient
	...
	if(!$mail->send()) {
		...
		$dba->_execute("UPDATE `fb_id` SET `last_send`= NOW() WHERE `fb_id`='".$fb_id."'");
